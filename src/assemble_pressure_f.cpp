...
void assemble_pressure_f_2d(double rho, double dx, double dy, double dt, 
                            ...
                            for (int i = 1; i < x_len_non_staggered; i++)
                            {
                                ...
                                for (int j = 1; j < y_len_non_staggered; j++)
                                {
                                    ...
                                    if (M_signed_distance(j_non_staggered,i - 1) > 0)//top
                                        P_TP.diagonal()[0] = 0;
                                    if (M_signed_distance(j_non_staggered,i + 1) > 0)//bot
                                        P_TP.diagonal()[1] = 0;
                                    if (M_signed_distance(j - 1,i_non_staggered) > 0)//right
                                        P_TP.diagonal()[2] = 0;
                                    if (M_signed_distance(j + 1,i_non_staggered) > 0)//left
                                        P_TP.diagonal()[3] = 0;
                                    Eigen::Vector4d q_j;
